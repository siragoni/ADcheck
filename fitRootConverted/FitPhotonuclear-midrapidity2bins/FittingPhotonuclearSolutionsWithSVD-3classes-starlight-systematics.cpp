...
Double_t FittingPhotonuclear(Int_t element, Int_t mode = 0, Int_t rap = 0)
{
  ...
  //==========================
  // SYSTEMATIC UNCERTAINTIES MIDRAPIDITY
  //==========================
  Double_t PartialSys08to02[3] = {5.3, 6.4, 6.5};
  Double_t PartialSys02to00[3] =  {5.3, 6.4, 6.5};
  //--------------------------
  // Pile-up
  Double_t PileUp08to02plus[3]  = {1.3, 3.8, 1.8};
  Double_t PileUp08to02minus[3] = {1.3, 3.8, 1.8};
  Double_t PileUp02to00plus[3]  = {1.3, 4.7, 1.9};
  Double_t PileUp02to00minus[3] = {1.3, 4.6, 1.9};
  //--------------------------
  // ZDC eff
  Double_t eZdc08to02plus[3]  = {1.5, 1.5, 10.2};
  Double_t eZdc08to02minus[3] = {1.6, 1.2, 12.0};
  Double_t eZdc02to00plus[3]  = {1.3, 1.8, 10.2};
  Double_t eZdc02to00minus[3] = {1.3, 1.6, 12.0};
  ...
  for (Int_t i = 0; i < 3; i++) {
    ...
    PartialSys08to02[i] = PartialSys08to02[i] * 0.01;
    PartialSys02to00[i] = PartialSys02to00[i] * 0.01;

    PileUp08to02plus[i]  = PileUp08to02plus[i] * 0.01;
    PileUp08to02minus[i] = PileUp08to02minus[i] * 0.01;
    PileUp02to00plus[i]  = PileUp02to00plus[i] * 0.01;
    PileUp02to00minus[i] = PileUp02to00minus[i] * 0.01;

    eZdc08to02plus[i]  = eZdc08to02plus[i] * 0.01;
    eZdc08to02minus[i] = eZdc08to02minus[i] * 0.01;
    eZdc02to00plus[i]  = eZdc02to00plus[i] * 0.01;
    eZdc02to00minus[i] = eZdc02to00minus[i] * 0.01;
  ...
  */
  ...
  if(         mode == 0 ){
    ...
    xsec4to25(0) = xsec4to25orig(0);
    xsec4to25(1) = xsec4to25orig(1)*(1. + 0.055);
    xsec4to25(2) = xsec4to25orig(2)*(1. + 0.110);
  } else if ( mode == 8) {
    xsec4to25(0) = xsec4to25orig(0);
    xsec4to25(1) = xsec4to25orig(1)*(1. - 0.055);
    xsec4to25(2) = xsec4to25orig(2)*(1. - 0.110);
  ...
  if(         mode == 0 ){
    ...
    xsec4to35(0) = xsec4to35orig(0);
    xsec4to35(1) = xsec4to35orig(1)*(1. + 0.055);
    xsec4to35(2) = xsec4to35orig(2)*(1. + 0.110);
  } else if ( mode == 8) {
    xsec4to35(0) = xsec4to35orig(0);
    xsec4to35(1) = xsec4to35orig(1)*(1. - 0.055);
    xsec4to35(2) = xsec4to35orig(2)*(1. - 0.110);
  ...
  if( mode == 1 ){
    ...
    xsec35to3(0) = xsec35to3orig(0);
    xsec35to3(1) = xsec35to3orig(1)*(1. + 0.055);
    xsec35to3(2) = xsec35to3orig(2)*(1. + 0.110);
  } else if ( mode == 8) {
    xsec35to3(0) = xsec35to3orig(0);
    xsec35to3(1) = xsec35to3orig(1)*(1. - 0.055);
    xsec35to3(2) = xsec35to3orig(2)*(1. - 0.110);
  ...
  if(         mode == 0 ){
    ...
    xsec3to25(0) = xsec3to25orig(0);
    xsec3to25(1) = xsec3to25orig(1)*(1. + 0.055);
    xsec3to25(2) = xsec3to25orig(2)*(1. + 0.110);
  } else if ( mode == 8) {
    xsec3to25(0) = xsec3to25orig(0);
    xsec3to25(1) = xsec3to25orig(1)*(1. - 0.055);
    xsec3to25(2) = xsec3to25orig(2)*(1. - 0.110);
  ...
  TVectorD xsec0(3);
  xsec0.Zero();
  xsec0(0) = Cent0N0N_2->GetY()[1];
  xsec0(1) = Cent0NXN_2->GetY()[1];
  xsec0(2) = CentXNXN_2->GetY()[1];
  // if( mode == 1 ){
  ...
  // photonuclear0.Print();
  if( mode == 1){  
    xsec05(0) = xsec05(0) *(1. + PartialSys08to02[0]);
    xsec05(1) = xsec05(1) *(1. + PartialSys08to02[1]);
    xsec05(2) = xsec05(2) *(1. + PartialSys08to02[2]);
    xsec0(0)  = xsec0(0)  *(1. + PartialSys02to00[0]);
    xsec0(1)  = xsec0(1)  *(1. + PartialSys02to00[1]);
    xsec0(2)  = xsec0(2)  *(1. + PartialSys02to00[2]);
  } else if (mode == 2){
    xsec05(0) = xsec05(0) *(1. - PartialSys08to02[0]);
    xsec05(1) = xsec05(1) *(1. - PartialSys08to02[1]);
    xsec05(2) = xsec05(2) *(1. - PartialSys08to02[2]);
    ...
    xsec05(0) = xsec05(0) *(1. + PileUp08to02plus[0]);
    xsec05(1) = xsec05(1) *(1. + PileUp08to02plus[1]);
    xsec05(2) = xsec05(2) *(1. + PileUp08to02plus[2]);
    xsec0(0)  = xsec0(0)  *(1. + PileUp02to00plus[0]);
    xsec0(1)  = xsec0(1)  *(1. + PileUp02to00plus[1]);
    xsec0(2)  = xsec0(2)  *(1. + PileUp02to00plus[2]);
  } else if (mode == 4){
    xsec05(0) = xsec05(0) *(1. - PileUp08to02minus[0]);
    xsec05(1) = xsec05(1) *(1. - PileUp08to02minus[1]);
    xsec05(2) = xsec05(2) *(1. - PileUp08to02minus[2]);
    xsec0(0)  = xsec0(0)  *(1. - PileUp02to00minus[0]);
    xsec0(1)  = xsec0(1)  *(1. - PileUp02to00minus[1]);
    xsec0(2)  = xsec0(2)  *(1. - PileUp02to00minus[2]);
  } else if (mode == 5){
    xsec05(0) = xsec05(0) *(1. + eZdc08to02plus[0]);
    xsec05(1) = xsec05(1) *(1. + eZdc08to02plus[1]);
    xsec05(2) = xsec05(2) *(1. + eZdc08to02plus[2]);
    xsec0(0)  = xsec0(0)  *(1. + eZdc02to00plus[0]);
    xsec0(1)  = xsec0(1)  *(1. + eZdc02to00plus[1]);
    xsec0(2)  = xsec0(2)  *(1. + eZdc02to00plus[2]);
  } else if (mode == 6){
    xsec05(0) = xsec05(0) *(1. - eZdc08to02minus[0]);
    xsec05(1) = xsec05(1) *(1. - eZdc08to02minus[1]);
    xsec05(2) = xsec05(2) *(1. - eZdc08to02minus[2]);
    xsec0(0)  = xsec0(0)  *(1. - eZdc02to00minus[0]);
    xsec0(1)  = xsec0(1)  *(1. - eZdc02to00minus[1]);
    xsec0(2)  = xsec0(2)  *(1. - eZdc02to00minus[2]);
  } else if (mode == 7){
    xsec05(0) = xsec05(0);
    xsec05(1) = xsec05(1) *(1. + 0.055);
    xsec05(2) = xsec05(2) *(1. + 0.110);
    xsec0(0)  = xsec0(0);
    xsec0(1)  = xsec0(1)  *(1. + 0.055);
    xsec0(2)  = xsec0(2)  *(1. + 0.110);
  } else if (mode == 8){
    xsec05(0) = xsec05(0);
    xsec05(1) = xsec05(1) *(1. - 0.055);
    xsec05(2) = xsec05(2) *(1. - 0.110);
...
void Computing(){
  Double_t HighSolutionx[8]  = {FittingPhotonuclear(0,1,0),FittingPhotonuclear(0,2,0),FittingPhotonuclear(0,3,0),FittingPhotonuclear(0,4,0),FittingPhotonuclear(0,5,0),FittingPhotonuclear(0,6,0),FittingPhotonuclear(0,7,0),FittingPhotonuclear(0,8,0)};
  Double_t LowSolutionx[8]   = {FittingPhotonuclear(1,1,0),FittingPhotonuclear(1,2,0),FittingPhotonuclear(1,3,0),FittingPhotonuclear(1,4,0),FittingPhotonuclear(1,5,0),FittingPhotonuclear(1,6,0),FittingPhotonuclear(1,7,0),FittingPhotonuclear(1,8,0)};
  ...
  Double_t HighSolution_fluxp  = HighSolutionx[6] - FittingPhotonuclear(0,0,0);
  Double_t HighSolution_fluxm  = HighSolutionx[7] - FittingPhotonuclear(0,0,0);
  ...
  Double_t LowSolution_fluxp   = LowSolutionx[6] - FittingPhotonuclear(1,0,0);
  Double_t LowSolution_fluxm   = LowSolutionx[7] - FittingPhotonuclear(1,0,0);




  Double_t HighSolution0x[8]  = {FittingPhotonuclear(0,1,1),FittingPhotonuclear(0,2,1),FittingPhotonuclear(0,3,1),FittingPhotonuclear(0,4,1),FittingPhotonuclear(0,5,1),FittingPhotonuclear(0,6,1),FittingPhotonuclear(0,7,1),FittingPhotonuclear(0,8,1)};
  Double_t LowSolution0x[8]   = {FittingPhotonuclear(1,1,1),FittingPhotonuclear(1,2,1),FittingPhotonuclear(1,3,1),FittingPhotonuclear(1,4,1),FittingPhotonuclear(1,5,1),FittingPhotonuclear(1,6,1),FittingPhotonuclear(1,7,1),FittingPhotonuclear(1,8,1)};
  ...
  Double_t HighSolution0_fluxp  = HighSolution0x[6] - FittingPhotonuclear(0,0,1);
  Double_t HighSolution0_fluxm  = HighSolution0x[7] - FittingPhotonuclear(0,0,1);
  ...
  Double_t LowSolution0_fluxp   = LowSolution0x[6] - FittingPhotonuclear(1,0,1);
  Double_t LowSolution0_fluxm   = LowSolution0x[7] - FittingPhotonuclear(1,0,1);



  Double_t HighSolution1x[8]  = {FittingPhotonuclear(0,1,2),FittingPhotonuclear(0,2,2),FittingPhotonuclear(0,3,2),FittingPhotonuclear(0,4,2),FittingPhotonuclear(0,5,2),FittingPhotonuclear(0,6,2),FittingPhotonuclear(0,7,2),FittingPhotonuclear(0,8,2)};
  Double_t LowSolution1x[8]   = {FittingPhotonuclear(1,1,2),FittingPhotonuclear(1,2,2),FittingPhotonuclear(1,3,2),FittingPhotonuclear(1,4,2),FittingPhotonuclear(1,5,2),FittingPhotonuclear(1,6,2),FittingPhotonuclear(1,7,2),FittingPhotonuclear(1,8,2)};
  ...
  Double_t HighSolution1_fluxp  = HighSolution1x[6] - FittingPhotonuclear(0,0,2);
  Double_t HighSolution1_fluxm  = HighSolution1x[7] - FittingPhotonuclear(0,0,2);
  ...
  Double_t LowSolution1_fluxp   = LowSolution1x[6] - FittingPhotonuclear(1,0,2);
  Double_t LowSolution1_fluxm   = LowSolution1x[7] - FittingPhotonuclear(1,0,2);





  Double_t HighSolution2x[8]  = {FittingPhotonuclear(0,1,3),FittingPhotonuclear(0,2,3),FittingPhotonuclear(0,3,3),FittingPhotonuclear(0,4,3),FittingPhotonuclear(0,5,3),FittingPhotonuclear(0,6,3),FittingPhotonuclear(0,7,3),FittingPhotonuclear(0,8,3)};
  Double_t LowSolution2x[8]   = {FittingPhotonuclear(1,1,3),FittingPhotonuclear(1,2,3),FittingPhotonuclear(1,3,3),FittingPhotonuclear(1,4,3),FittingPhotonuclear(1,5,3),FittingPhotonuclear(1,6,3),FittingPhotonuclear(1,7,3),FittingPhotonuclear(1,8,3)};
  ...
  Double_t HighSolution2_fluxp  = HighSolution2x[6] - FittingPhotonuclear(0,0,3);
  Double_t HighSolution2_fluxm  = HighSolution2x[7] - FittingPhotonuclear(0,0,3);
  ...
  Double_t LowSolution2_fluxp   = LowSolution2x[6] - FittingPhotonuclear(1,0,3);
  Double_t LowSolution2_fluxm   = LowSolution2x[7] - FittingPhotonuclear(1,0,3);
  ...
  cout << "Element 0 (-4 < y < -2.5)= " << HighSolution << ", " << HighSolution_corr << ", " << HighSolution_pileup << ", " << HighSolution_ezdc << ", " << HighSolution_fluxp << ", " << HighSolution_fluxm << endl;
  cout << "Element 1 (-4 < y < -2.5)= " << LowSolution  << ", " << LowSolution_corr  << ", " << LowSolution_pileup  << ", " << LowSolution_ezdc  << ", " << LowSolution_fluxp << ", " << LowSolution_fluxm << endl;

  cout << "Element 0 (-4 < y < -3.5)= " << HighSolution0 << ", " << HighSolution0_corr << ", " << HighSolution0_pileup << ", " << HighSolution0_ezdc << ", " << HighSolution0_fluxp << ", " << HighSolution0_fluxm << endl;
  cout << "Element 1 (-4 < y < -3.5)= " << LowSolution0  << ", " << LowSolution0_corr  << ", " << LowSolution0_pileup  << ", " << LowSolution0_ezdc  << ", " << LowSolution0_fluxp << ", " << LowSolution0_fluxm << endl;

  cout << "Element 0 (-3.5 < y < -3)= " << HighSolution1 << ", " << HighSolution1_corr << ", " << HighSolution1_pileup << ", " << HighSolution1_ezdc << ", " << HighSolution1_fluxp << ", " << HighSolution1_fluxm << endl;
  cout << "Element 1 (-3.5 < y < -3)= " << LowSolution1  << ", " << LowSolution1_corr  << ", " << LowSolution1_pileup  << ", " << LowSolution1_ezdc  << ", " << LowSolution1_fluxp << ", " << LowSolution1_fluxm << endl;

  cout << "Element 0 (-3 < y < -2.5)= " << HighSolution2 << ", " << HighSolution2_corr << ", " << HighSolution2_pileup << ", " << HighSolution2_ezdc << ", " << HighSolution2_fluxp << ", " << HighSolution2_fluxm << endl;
  cout << "Element 1 (-3 < y < -2.5)= " << LowSolution2  << ", " << LowSolution2_corr  << ", " << LowSolution2_pileup  << ", " << LowSolution2_ezdc  << ", " << LowSolution2_fluxp << ", " << LowSolution2_fluxm << endl;
  ...
  Double_t HighSolutionMid08to02[8]  = {FittingPhotonuclear(0,1,7),FittingPhotonuclear(0,2,7),FittingPhotonuclear(0,3,7),FittingPhotonuclear(0,4,7),FittingPhotonuclear(0,5,7),FittingPhotonuclear(0,6,7),FittingPhotonuclear(0,7,7),FittingPhotonuclear(0,8,7)};
  Double_t LowSolutionMid08to02[8]   = {FittingPhotonuclear(1,1,7),FittingPhotonuclear(1,2,7),FittingPhotonuclear(1,3,7),FittingPhotonuclear(1,4,7),FittingPhotonuclear(1,5,7),FittingPhotonuclear(1,6,7),FittingPhotonuclear(1,7,7),FittingPhotonuclear(1,8,7)};

  Double_t HighSolutionMid08to02_corr   = 0.5*(HighSolutionMid08to02[0]-HighSolutionMid08to02[1]);
  Double_t HighSolutionMid08to02_pileup = 0.5*(HighSolutionMid08to02[2]-HighSolutionMid08to02[3]);
  Double_t HighSolutionMid08to02_ezdc   = 0.5*(HighSolutionMid08to02[4]-HighSolutionMid08to02[5]);
  Double_t HighSolutionMid08to02_fluxp  = HighSolutionMid08to02[6] - FittingPhotonuclear(0,0,7);
  Double_t HighSolutionMid08to02_fluxm  = HighSolutionMid08to02[7] - FittingPhotonuclear(0,0,7);
  Double_t LowSolutionMid08to02_corr    = 0.5*(LowSolutionMid08to02[0]-LowSolutionMid08to02[1]);
  Double_t LowSolutionMid08to02_pileup  = 0.5*(LowSolutionMid08to02[2]-LowSolutionMid08to02[3]);
  Double_t LowSolutionMid08to02_ezdc    = 0.5*(LowSolutionMid08to02[4]-LowSolutionMid08to02[5]);
  Double_t LowSolutionMid08to02_fluxp   = LowSolutionMid08to02[6] - FittingPhotonuclear(1,0,7);
  Double_t LowSolutionMid08to02_fluxm   = LowSolutionMid08to02[7] - FittingPhotonuclear(1,0,7);
  ...
  Double_t HighSolutionMid02to00[8]  = {FittingPhotonuclear(0,1,8),FittingPhotonuclear(0,2,8),FittingPhotonuclear(0,3,8),FittingPhotonuclear(0,4,8),FittingPhotonuclear(0,5,8),FittingPhotonuclear(0,6,8),FittingPhotonuclear(0,7,8),FittingPhotonuclear(0,7,8)};
  Double_t LowSolutionMid02to00[8]   = {FittingPhotonuclear(1,1,8),FittingPhotonuclear(1,2,8),FittingPhotonuclear(1,3,8),FittingPhotonuclear(1,4,8),FittingPhotonuclear(1,5,8),FittingPhotonuclear(1,6,8),FittingPhotonuclear(1,7,8),FittingPhotonuclear(1,7,8)};

  Double_t HighSolutionMid02to00_corr   = 0.5*(HighSolutionMid02to00[0]-HighSolutionMid02to00[1]);
  Double_t HighSolutionMid02to00_pileup = 0.5*(HighSolutionMid02to00[2]-HighSolutionMid02to00[3]);
  Double_t HighSolutionMid02to00_ezdc   = 0.5*(HighSolutionMid02to00[4]-HighSolutionMid02to00[5]);
  Double_t HighSolutionMid02to00_fluxp  = HighSolutionMid02to00[6] - FittingPhotonuclear(0,0,8);
  Double_t HighSolutionMid02to00_fluxm  = HighSolutionMid02to00[7] - FittingPhotonuclear(0,0,8);
  Double_t LowSolutionMid02to00_corr    = 0.5*(LowSolutionMid02to00[0]-LowSolutionMid02to00[1]);
  Double_t LowSolutionMid02to00_pileup  = 0.5*(LowSolutionMid02to00[2]-LowSolutionMid02to00[3]);
  Double_t LowSolutionMid02to00_ezdc    = 0.5*(LowSolutionMid02to00[4]-LowSolutionMid02to00[5]);
  Double_t LowSolutionMid02to00_fluxp   = LowSolutionMid02to00[6] - FittingPhotonuclear(1,0,8);
  Double_t LowSolutionMid02to00_fluxm   = LowSolutionMid02to00[7] - FittingPhotonuclear(1,0,8);

  cout << "Element 0 (y = 0.5) true = " << HighSolution05 << " , " << HighSolutionMid08to02_corr << ", " << HighSolutionMid08to02_pileup << ", " << HighSolutionMid08to02_ezdc << ", " << HighSolutionMid08to02_fluxp << ", " << HighSolutionMid08to02_fluxm << endl;
  cout << "Element 1 (y = 0.5) true = " << LowSolution05  << " , " << LowSolutionMid08to02_corr  << ", " << LowSolutionMid08to02_pileup  << ", " << LowSolutionMid08to02_ezdc  << ", " << LowSolutionMid08to02_fluxp << ", " << LowSolutionMid08to02_fluxm << endl;

  cout << "Element 0 (y = 0.)  true = "  << HighSolution0X << " , " << HighSolutionMid02to00_corr << ", " << HighSolutionMid02to00_pileup << ", " << HighSolutionMid02to00_ezdc << ", " << HighSolutionMid02to00_fluxp << ", " << HighSolutionMid02to00_fluxm << endl;
  cout << "Element 1 (y = 0.)  true = "  << LowSolution0X  << " , " << LowSolutionMid02to00_corr  << ", " << LowSolutionMid02to00_pileup  << ", " << LowSolutionMid02to00_ezdc  << ", " << LowSolutionMid02to00_fluxp << ", " << LowSolutionMid02to00_fluxm << endl;
  ...
  Double_t ey[12]   = {HighSolution_corr, HighSolution0_corr, HighSolution1_corr, HighSolution2_corr, LowSolution_corr, LowSolution0_corr, LowSolution1_corr, LowSolution2_corr, HighSolutionMid08to02_corr, HighSolutionMid02to00_corr, LowSolutionMid02to00_corr, LowSolutionMid08to02_corr };
  Double_t ey2[12]  = {HighSolution_pileup, HighSolution0_pileup, HighSolution1_pileup, HighSolution2_pileup, LowSolution_pileup, LowSolution0_pileup, LowSolution1_pileup, LowSolution2_pileup, HighSolutionMid08to02_pileup, HighSolutionMid02to00_pileup, LowSolutionMid02to00_pileup, LowSolutionMid08to02_pileup };
  Double_t ey3[12]  = {HighSolution_ezdc, HighSolution0_ezdc, HighSolution1_ezdc, HighSolution2_ezdc, LowSolution_ezdc, LowSolution0_ezdc, LowSolution1_ezdc, LowSolution2_ezdc, HighSolutionMid08to02_ezdc, HighSolutionMid02to00_ezdc, LowSolutionMid02to00_ezdc, LowSolutionMid08to02_ezdc };
