...
public:
  ...
  TGraphErrors *get_onon_xnxn();
  TGraphErrors *get_onon_xnon();
  void printData();
};//alice
alice alice::alic;
...
TGraphErrors *alice::get_onon_xnon() {
  //forward data
  const Int_t   np        = 6;
  const Double_t w[np]    = {ALICE_photo_W[0], ALICE_photo_W[1], ALICE_photo_W[2], ALICE_photo_W[3], ALICE_photo_W[4], ALICE_photo_W[5] };
  const Double_t sig[np]  = {0.00881, 0.01395, 0.01723, 0.03238, 0.04049, 0.07075};
  const Double_t stat[np] = {0.00032, 0.00029, 0.00081, 0.01170, 0.01248, 0.04088};
  const Double_t sys[np]  = {0.00064, 0.00576, 0.00124, 0.00324, 0.00380, 0.00576};
  double err[np] = {0,0,0, 0,0,0};
  for (size_t i = 0; i < np; i++)
  {
    err[i] = TMath::Sqrt(stat[i]*stat[i] + sys[i]*sys[i]);
  }
...
TGraphErrors *alice::get_onon_xnxn() {
  //forward data
  const Int_t   np        = 6;
  const Double_t w[np]    = {ALICE_photo_W[0], ALICE_photo_W[1], ALICE_photo_W[2], ALICE_photo_W[3], ALICE_photo_W[4], ALICE_photo_W[5] };
  const Double_t sig[np]  = {0.00881, 0.01383, 0.01659, 0.05777, 0.05840, 0.05993};
  const Double_t stat[np] = {0.00031, 0.00027, 0.00079, 0.01086, 0.01017, 0.03281};
  const Double_t sys[np]  = {0.00063, 0.00096, 0.00106, 0.01044, 0.01245, 0.01666};
  double err[np] = {0,0,0, 0,0,0};
  for (size_t i = 0; i < np; i++)
  {
    err[i] = TMath::Sqrt(stat[i]*stat[i] + sys[i]*sys[i]);
